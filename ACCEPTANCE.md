...
| N-7 | An admin declaring `list_filter = (...)` causes the SPA to surface filters as a sidebar drawer (`≥1024px`) / bottom-sheet (`<1024px`); applied filters appear as a chip row above the table; the URL contains the filter state so deep-links round-trip. (Closes [#56](https://github.com/MartinCastroAlvarez/django-admin-react/issues/56), backend shipped via [PR #99](https://github.com/MartinCastroAlvarez/django-admin-react/pull/99); SPA side awaits the frontend implementation PR.) | Add `list_filter` to an example; observe filter chips + URL state. |
| N-8 | An admin declaring `date_hierarchy = "<field>"` causes the SPA to surface a year→month→day drill-down strip above the filter sidebar (`≥1024px`) / under the page header (`<1024px`); state threads into N-7's chip row. (Closes [#62](https://github.com/MartinCastroAlvarez/django-admin-react/issues/62), backend shipped via [PR #80](https://github.com/MartinCastroAlvarez/django-admin-react/pull/80); SPA side awaits the frontend implementation PR.) | Add `date_hierarchy` to an example; drill down; observe URL state. |
| N-9 | When the visible sidebar contains 8 or more registered models, the sidebar surfaces a **text input** above the list. Typing into the input narrows the list to entries whose app label or model name contains the typed string (case-insensitive). Filtering is purely client-side (no API round-trip; reads from `@dar/data`'s registry cache). `Esc` or clearing the input restores the full list. Filter state is per-session (not persisted to URL or storage). Matches the legacy `django.contrib.admin` sidebar Filter affordance. (Tracks [#133](https://github.com/MartinCastroAlvarez/django-admin-react/issues/133); SPA side awaits the frontend implementation PR.) | Register 8 models across 2 apps; type `bal` in the sidebar filter; only matching entries remain; `Esc` restores full list. |
| N-10 | The sidebar emits **apps sorted alphabetically by `verbose_name`** (case-insensitive). Within each app, **models are sorted alphabetically by `verbose_name_plural`**. Order is stable across reloads regardless of `INSTALLED_APPS` order or `admin.site.register()` call order — matches `AdminSite.get_app_list(request)` semantics, so a consumer's override of `get_app_list` automatically wins. (Tracks [#136](https://github.com/MartinCastroAlvarez/django-admin-react/issues/136); awaits the backend `api/registry.py` switch from `_registry` iteration to `get_app_list`.) | Register apps `zebra`, `apple`, `mango`; observe sidebar order `Apple → Mango → Zebra`. |
...
- N-5 (session-expiry modal) depends on the backend emitting
  `error.code: "session_expired"` (Engineering — see
  [`docs/api-contract.md`](docs/api-contract.md) §10) and on
  `@dar/api` routing that envelope to a global `<SessionGate />`
...
| S-5 | The package ships **no parallel auth *mechanism***: no OAuth, no JWT issuance, no custom credential hashing or token minting, no `User`/`Group`/`Permission` of its own. It **may** expose thin JSON entry points that **delegate** entirely to Django's own auth — React login/logout (`views/auth.py`, PRs #167/#168/#120) and admin password-set (`views/password.py`, #252) — over `authenticate` / `login` / `logout` / `AdminPasswordChangeForm` / `user.set_password`. Auth remains fully Django + the consumer's `AdminSite`; these are UI shells, not an auth framework. | `tests/test_security.py::test_s5_no_parallel_auth_mechanism_in_views` — no view references `jwt` / `oauth`, and none mints credentials itself (`make_password`, `set_unusable_password`, `jwt.encode`, `secrets.token_*`, `create_access_token`, `itsdangerous`). See the ADR in `docs/agents/decisions.md`. |
