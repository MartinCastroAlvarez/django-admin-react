...
| E-6a | Adding `actions = [my_action]` to an existing `ModelAdmin` causes the SPA list page to show the action dropdown + checkbox column, with no frontend change. (X-2.) | Add `make_published` to `Account`; reload list page. *Gate: Architect signs off on the action invocation endpoint shape.* |
| E-6b | An action invocation respects `ModelAdmin.has_*_permission` server-side; the SPA does not even render the action if the user lacks the perm. (X-2.) | Toggle perm, observe. *Gate: Security signs off on the perm enforcement path.* |
...
| E-7a | Adding `inlines = [BookInline]` to an existing `ModelAdmin` causes the SPA detail page to render the inline section, with no frontend change. (X-4.) | Add an inline to `Author`; open an author's detail. *Gate: Architect signs off on the inline payload shape.* |
...
| E-8a | Returning a non-empty `get_detail_blocks(request, obj)` from a `ModelAdmin` causes the SPA detail page to render the blocks in their declared `placement` slot. (X-5.) | Add a `stats` block; observe. *Gate: Architect signs off on the block schema enum.* |
...
| E-9  | A `type: "html"` block runs through the configured server-side sanitiser before reaching the SPA; `<script>` tags and inline event handlers never survive the round-trip. (X-6.) | Try to slip a `<script>` through; observe stripped output. **Gate: Security signs off on the sanitiser library, allowlist, and `allow_unsafe_html` setting.** Drafted, not live, until that sign-off. |
