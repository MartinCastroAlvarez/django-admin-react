...
Real SPA screenshots will appear here once the React shell renders
end-to-end against `examples/library/` — tracked under
[#87](https://github.com/MartinCastroAlvarez/django-admin-react/issues/87).
The screenshot inventory + naming contract is already in
[`docs/screenshots/README.md`](docs/screenshots/README.md); the
capture work blocks on the SPA mount detection fix
([#113](https://github.com/MartinCastroAlvarez/django-admin-react/issues/113))
and the v0.2 directives — dark mode
([#84](https://github.com/MartinCastroAlvarez/django-admin-react/issues/84)),
mobile patterns
([#85](https://github.com/MartinCastroAlvarez/django-admin-react/issues/85)),
PWA install prompt
([#86](https://github.com/MartinCastroAlvarez/django-admin-react/issues/86)).

The legacy HTML admin images that previously lived here misrepresented
the package — they showed Django's own admin against the example
apps, not the React SPA this package ships — see
[#115](https://github.com/MartinCastroAlvarez/django-admin-react/issues/115).
They've been removed until the SPA capture pipeline produces real
pixels.
